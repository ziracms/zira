...
var dash_editor_html_drop = function(element) {
    ...
    if (element.parent=='files' && element.type=='image') {
        ...
        html='<img class="image" src="'+baseUrl(element.data.replace(regexp,'/'))+'" alt="'+element.title+'" width="'+width+'" height="'+height+'" />';
    } else if (element.parent=='files' && element.type!='folder') {
        html='<a href="'+baseUrl(element.data)+'" title="'+element.title+'" download="'+element.title+'">'+element.title+'</a>';
...
var dash_editor_html_load = function() {
    ...
    tinymce.init({
        ...
        toolbar: ['desk_save | undo redo | table | bullist numlist | desk_file_selector  image media link | outdent indent | hr pagebreak | code', 'styleselect | bold italic underline removeformat |  aligncenter alignleft alignright alignjustify '],
        menubar: false,
            ...
            editor.addButton('desk_file_selector',{ 'icon': 'browse', 'title':t('File manager'), 'onclick': this.bind(this,function(){
                ...
                for (var i=0; i<elements.length; i++){
                    ...
                    if (element.type=='image') {
                        ...
                        if (typeof(element.image_height)!="undefined") height=element.image_height;
                        var regexp = new RegExp('\\'+desk_ds, 'g');
                        html+='<img src="'+baseUrl(element.data.replace(regexp,'/'))+'" alt="'+element.title+'" width="'+width+'" height="'+height+'" class="image" />';
                    } else if (element.type!='folder') html+='<a href="'+baseUrl(element.data)+'" title="'+element.title+'" download="'+element.title+'">'+element.title+'</a>';
                        if (html.length>0){
                    this.editor.execCommand('mceInsertContent', false, html);
                    }
                }
