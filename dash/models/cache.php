...
class Cache extends Model {
    public function info($file) {
        ...
        if (!file_exists(ROOT_DIR . DIRECTORY_SEPARATOR . CACHE_DIR . DIRECTORY_SEPARATOR . $file)) return array();
        $is_dir = is_dir(ROOT_DIR . DIRECTORY_SEPARATOR . CACHE_DIR . DIRECTORY_SEPARATOR . $file);

        $info = array();
        //$info[]='<span class="glyphicon glyphicon-tag"></span> '.Zira\Helper::html($file);
        $info[]='<span class="glyphicon glyphicon-time"></span> '.date(Zira\Config::get('date_format'),filemtime(ROOT_DIR . DIRECTORY_SEPARATOR . CACHE_DIR . DIRECTORY_SEPARATOR . $file));
        if (!$is_dir) {
            $info[]='<span class="glyphicon glyphicon-hdd"></span> '.number_format(filesize(ROOT_DIR . DIRECTORY_SEPARATOR . CACHE_DIR . DIRECTORY_SEPARATOR . $file) / 1024, 2).' kB';
        }
        return $info;
    }
    ...
    public function delete($data) {
        ...
        foreach($data as $item) {
            ...
            $path = ROOT_DIR . DIRECTORY_SEPARATOR . CACHE_DIR . DIRECTORY_SEPARATOR . $item;
            if (!file_exists($path)) return array('error' => Zira\Locale::t('An error occurred'));
            if (is_dir($path)) {
                $dirs[]=$path;
                $stack = array(CACHE_DIR . DIRECTORY_SEPARATOR . $item);
                while(count($stack)>0) {
                    $d = array_shift($stack);
