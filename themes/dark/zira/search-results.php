...
<ul class="list<?php if (!empty($settings['grid']) && empty($settings['simple'])) echo ' grid-category-wrapper grid-col-'.(intval($settings['grid'])+1) ?><?php if (isset($class)) echo ' '.$class ?>">
<?php $co=0; ?>
...
<button class="btn btn-primary search-results-view-more" type="button" data-url="<?php echo Zira\Helper::url('search') ?>" data-text="<?php echo Zira\Helper::html($settings['text']) ?>" data-offset="<?php echo Zira\Helper::html($settings['offset']+$co) ?>"><?php echo t('View more') ?>&nbsp;&rsaquo;&rsaquo;</button>
</div>
<?php endif; ?>
<?php endif; ?>
