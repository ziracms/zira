...
<ul class="list<?php if (!empty($settings['grid'])) echo ' grid-category-wrapper grid-col-'.(intval($settings['grid'])+1) ?><?php if (isset($class)) echo ' '.$class ?>">
<?php $co=0; ?>
<?php foreach($records as $record): ?>
<li class="list-item <?php echo $record->thumb ? 'with-thumb' : 'no-thumb' ?><?php echo $record->published!=Zira\Models\Record::STATUS_PUBLISHED ? ' unpublished' : '' ?>">
...
<?php if (isset($settings) && !empty($settings['limit']) && count($records)>$settings['limit'] && isset($settings['offset'])): ?>
<div class="fields-search-results-view-more-wrapper">
<button class="btn btn-primary fields-search-results-view-more" type="button" data-url="<?php echo Zira\Helper::url('fields/search') ?>" data-offset="<?php echo Zira\Helper::html($settings['offset']+$co) ?>"><?php echo t('View more') ?>&nbsp;&rsaquo;&rsaquo;</button>
</div>
<?php endif; ?>
<?php endif; ?>
